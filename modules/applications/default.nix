...
in
{
  ...
  options = with lib; {
    ...
    namespace =
      let
        namespaceSubmodule =
          recursiveUpdate (globalSubmoduleOf "io.k8s.apimachinery.pkg.apis.meta.v1.ObjectMeta")
            {
              config = {
                metadata = {
      ...
      in
      mkOption {
        type = types.either types.str namespaceSubmodule;
        default = config.name;
        description = "Namespace to deploy application into (defaults to name). It is possible to specify an ObjectMeta";
      };
